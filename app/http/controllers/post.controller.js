...
class PostController extends Controller {
  ...
  //   let dbQuery = {};
  //   const user = req.user;
  //   let { search, categorySlug, categoryId, sort, page, limit } = req.query;
  ...
  //   // فیلتر بر اساس categoryId یا categorySlug
  //   if (categoryId || categorySlug) {
  //     const categoryIds = [];

  //     // اگر categoryId ارسال شده باشد
  //     if (categoryId) {
  //       const ids = [categoryId].flat(2);
  //       categoryIds.push(...ids.map((id) => mongoose.Types.ObjectId(id)));
  //     }

  //     // اگر categorySlug ارسال شده باشد
  //     if (categorySlug) {
  //       const categories = [categorySlug].flat(2);
  //       for (const item of categories) {
  //         const { _id } = await CategoryModel.findOne({ slug: item });
  //         if (_id) categoryIds.push(_id);
  //       }
  //     }
  ...
  async getPostByCategory(req, res) {
    const { id } = req.params;
    const posts = await PostModel.find({ category: id }).populate([
      {
        path: "author",
        model: "User",
        select: { name: 1, biography: 1, avatar: 1 },
    ...
    if (!posts.length)
      throw createHttpError.NotFound("هیچ پستی در این دسته‌بندی یافت نشد");
